"""GPU-centered helpers for source ore multiclass segmentation training."""
...
from ore_detection.training.const import SOURCE_ORE_TRAIN_RGB_MEAN, SOURCE_ORE_TRAIN_RGB_STD
...
def _require_torch():
    try:
        ...
        raise RuntimeError("PyTorch is required for source ore GPU training helpers.") from exc
    return torch, F


@dataclass(frozen=True)
class DownsampledOreSample:
    dataset: str
    split: str
    stem: str
    image_path: Path
...
def list_downsampled_source_ore_samples(
    root: str | Path = "data_work/source_ore_downsampled",
    *,
    split: str | None = None,
) -> list[DownsampledOreSample]:
    """List paired downsampled source ore images and class-index masks."""
    root = Path(root)
    image_root = root / "images"
    mask_root = root / "masks"
    samples: list[DownsampledOreSample] = []
...
def load_class_index_mask(path: str | Path):
    """Load a compact class-index mask tensor shaped ``[H,W]``."""
    torch, _ = _require_torch()
    try:
    ...
    if mask.ndim != 2:
        raise ValueError(f"class-index mask must be shaped [H,W], got {tuple(mask.shape)} for {path}")
    return mask.to(dtype=torch.long)


def load_cached_tensors(
    samples: Sequence[DownsampledOreSample],
    *,
    device: str | object,
    image_dtype=None,
    pin_memory: bool = False,
) -> dict[str, object]:
    """Load samples into stacked tensors, optionally directly on a GPU device."""
    torch, _ = _require_torch()
    if not samples:
    ...
    return {"image": image_tensor, "class_index": mask_tensor, "samples": list(samples)}
...
def iter_cached_batches(
    ...
    *,
    ...
    """Yield batches from tensors returned by ``load_cached_tensors``."""
    torch, _ = _require_torch()
    images = cached["image"]
    masks = cached["class_index"]
    ...
    for start in range(0, sample_count, batch_size):
        ...
        yield {
            ...
            "class_index": masks.index_select(0, index).long(),
...
def normalize_images(
    images,
    *,
    mean: tuple[float, float, float] = SOURCE_ORE_TRAIN_RGB_MEAN,
    std: tuple[float, float, float] = SOURCE_ORE_TRAIN_RGB_STD,
):
...
def class_index_to_one_hot(class_index, *, class_count: int):
    """Convert ``[B,H,W]`` class-index masks to ``[B,C,H,W]`` one-hot masks."""
    _, F = _require_torch()
    return F.one_hot(class_index.long(), num_classes=class_count).permute(0, 3, 1, 2).float()


def multiclass_dice(probs, target_one_hot, eps: float = 1e-6):
    """Return one Dice score per class for a multiclass probability tensor."""
    dims = (0, 2, 3)
...
def binary_iou_from_class_index(pred_class, target_class, *, background_index: int, eps: float = 1e-6):
    """Calculate binary ore IoU by treating every non-background class as ore."""
    pred_binary = pred_class != background_index
    target_binary = target_class != background_index
    intersection = (pred_binary & target_binary).sum().float()
    union = (pred_binary | target_binary).sum().float()
    return (intersection + eps) / (union + eps)


def _pad_to_minimum(image, mask, *, target_size: int, background_index: int):
    ...
    mask = F.pad(mask.unsqueeze(0).unsqueeze(0), (left, right, top, bottom), value=background_index).squeeze(0).squeeze(0)
    return image, mask
...
def _random_crop(image, mask, *, output_size: int):
    ...
    if height == output_size:
        ...
        top = int(torch.randint(0, height - output_size + 1, (), device=image.device).item())
    if width == output_size:
        left = 0
    else:
        left = int(torch.randint(0, width - output_size + 1, (), device=image.device).item())
    return image[:, top : top + output_size, left : left + output_size], mask[top : top + output_size, left : left + output_size]


def augment_source_ore_batch(
    images,
    class_index,
    *,
    output_size: int,
    ...
    """Apply train-only GPU augmentation to images and class-index masks."""
    torch, F = _require_torch()
    if images.ndim != 4:
    ...
    for sample_index in range(batch_size):
        ...
        if rotation:
            ...
            mask = torch.rot90(mask, rotation, dims=(0, 1))
        ...
        image, mask = _pad_to_minimum(image, mask, target_size=output_size, background_index=background_index)
